 ...
 */
@Slf4j
public class GoogleSkanFileHandler implements Handler<Buffer> {
    ...
    private final Promise<List<Summary>> donePromise;
    private final Buffer buffer = Buffer.buffer();

    public GoogleSkanFileHandler(Promise<List<Summary>> promise) {
    ...
    public void end() {
        Arrays.stream(buffer.toString("unicode").split("\n")).forEach(line -> getLineHandler().handle(line));
